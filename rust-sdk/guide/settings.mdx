...
let settings = client.settings().fetch_deployment_settings()
    .send()
    .await?;
...
client.settings().update_authentication_settings(AuthenticationSettings {
...
client.settings().update_display_settings(DisplaySettings {
...
```rust
use wacht::WachtClient;
use wacht::models::DeploymentB2bSettingsUpdates;

let client = WachtClient::new("wk_live_...");

client.settings().update_b2b_settings(DeploymentB2bSettingsUpdates {
    enabled: Some(true),
    allow_organizations: Some(true),
    allow_workspaces: Some(true),
    ..Default::default()
})
.send()
.await?;
...
let templates = client.settings().fetch_jwt_templates()
    .send()
    .await?;
...
let template = client.settings().create_jwt_template(CreateJwtTemplateRequest {
...
let template = client.settings().update_jwt_template(
    "template_id",
...
client.settings().delete_jwt_template("template_id")
    .send()
    .await?;
```
...
let config = client.settings().update_smtp_config(SmtpConfigRequest {
...
let result = client.settings().verify_smtp_connection(SmtpConfigRequest {
    host: "smtp.example.com".to_string(),
    port: 587,
    username: "user@example.com".to_string(),
    password: "secret".to_string(),
...
println!("SMTP connection: {}", result.success); // SmtpVerifyResponse structure assumes `success` exists
```
...
client.settings().remove_smtp_config()
    .send()
    .await?;
```
...
let template = client.settings().fetch_email_template("email_verification")
    .send()
    .await?;
...
client.settings().update_email_template("email_verification", EmailTemplate {
...
let connections = client.settings().fetch_social_connections()
    .send()
    .await?;
...
let connection = client.settings().upsert_social_connection(SocialConnection {
...
let result = client.settings().upload_image("logo", image_data, "logo.png".to_string())
    .send()
    .await?;
...
match client.settings().fetch_deployment_settings().send().await {
