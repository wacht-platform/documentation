...
1. Create and update agents with backend SDK methods (`client.ai.createAgent`, `client.ai.createActorProject`).
2. Manage deployment MCP servers through backend SDK HTTP helpers against `/ai/mcp-servers`. Runtime user connections use the actor-scoped MCP runtime APIs.
3. Create actor projects and run threads from your app with tickets and frontend hooks.
...
3. Connect MCP servers at runtime through the actor-scoped MCP runtime APIs.
...
4. [Frontend AI Runtime Routes](/guides/ai-agents/frontend-api-runtime-routes)
