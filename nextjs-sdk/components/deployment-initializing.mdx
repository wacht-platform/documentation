...
```tsx
import { DeploymentInitializing } from "@wacht/nextjs";
```
...
```tsx
import { DeploymentInitializing, DeploymentInitialized } from "@wacht/nextjs";

export default function RootApplicationLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      {/* 
