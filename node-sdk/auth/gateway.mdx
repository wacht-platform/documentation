...
When resolving gateway metadata for OAuth access tokens, resource fields are split:

- `metadata.oauth_resource`: the original `resource` value from the OAuth authorization request.
- `metadata.granted_resource`: the effective resource that was actually granted and used for authz checks.

Gateway response headers expose the same split:
