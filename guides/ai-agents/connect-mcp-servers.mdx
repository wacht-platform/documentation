...
description: "Set up MCP servers, choose the right auth mode, and handle runtime connection behavior in your app."
---
...
## Step 3: Runtime connect/disconnect in user session

MCP server registration is deployment control-plane. Runtime connect/disconnect is actor-scoped and happens per active agent session.
...
Runtime connection is already handled by `useActorMcpServers` hooks.
...
2. [Frontend AI Runtime Routes](/guides/ai-agents/frontend-api-runtime-routes)
3. [React Router `useActorMcpServers`](/react-router-sdk/hooks/use-actor-mcp-servers)
